...
import $ from "jquery";
import roulette from "../sound/roulette.mp3";
...
const Context = (props) => {
  ...
  useEffect(() => {
    ...
    function playRoulette() {
      const rouletteMusic = new Audio(roulette)
  ...
  const bet = (e, num, stat) => {
        ...
        // Add this check to prevent adding more than four chips
        if (tempAry.betAmount >= chip * 4) {
          if (stat === "add") {
            return;
          }
        }
  ...
  const doubleBet = () => {
    if ((balance + totalBet) >= (totalBet * 2) && totalBet < 300) {
